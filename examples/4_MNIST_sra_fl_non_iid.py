...
#                           config=configs) 
#            for idx_benign in range(num_benign)]
clients = [Benign_clients(model=CNN_MNIST().to(configs['device']),
                          dataloader=Non_iid_dataloader[idx_benign],
                          config=configs) 
           for idx_benign in range(num_benign)]
...
model_global = CNN_MNIST().to(configs['device'])
...
for epoch_ in range(configs['num_epoch']):
    ...
    if (epoch_+1)%configs['time_step'] == 0:
