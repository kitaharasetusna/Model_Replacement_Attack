...
from my_utils.utils_dataloader import get_ds_cifar100 
from my_utils.utils_reading_disks import get_dict_from_yaml

if __name__ == '__main__':

    # -------------------------------------- 0. load config ------------------
    path_config = '../configs/7_cifar_100_sra_fl_non_iid.yaml'
    configs = get_dict_from_yaml(path=path_config)
    print(configs)
    # -------------------------------------- 0. load config ------------------ 

    # --------------------------------------1. load datasets  ---------------- 
    ds_train, ds_test = get_ds_cifar100()
    # --------------------------------------1. load datasets  ----------------
    ...
    if configs['load_idx']==True:
        ...
        # alpha for the Dirichlet distribution
            ...
            data_dict = iid_partition(ds_train, configs['num_clients'])  # Uncomment for idd_partition
